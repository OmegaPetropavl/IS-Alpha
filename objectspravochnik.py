...
"}")
        ...
        self.editbutton.clicked.connect(lambda: self.openEditObject())
    ...
    def openEditObject(self):
        selected_row = self.tableobjects.currentRow()
        if selected_row < 0:
            QMessageBox.warning(self.background, "Ошибка", "Выберите объект для редактирования")
        else:
            
            edit_object = QtWidgets.QDialog()
            ui = Ui_EditObject()
            ui.setupUi(edit_object,selected_row)
        
        # Установка значений полей в окне редактирования
         
            global Adres1
            Adres1 = (self.tableobjects.item(selected_row, 1).text())
            with open('adres1.pkl', 'wb') as file:
                pickle.dump(Adres1, file)
            ui.name.setText(self.tableobjects.item(selected_row, 0).text())
            ui.adress.setText(self.tableobjects.item(selected_row, 1).text())
            ui.square.setText(self.tableobjects.item(selected_row, 2).text())
            ui.podva_square.setText(self.tableobjects.item(selected_row, 4).text())
            ui.koef_podvl.setText(self.tableobjects.item(selected_row, 5).text())
            ui.koef_tech.setText(self.tableobjects.item(selected_row, 6).text())
